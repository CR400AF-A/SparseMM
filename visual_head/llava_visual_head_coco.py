...
def parse_coco_data(data):
    parsed_pairs = []
    for id in range(len(data["bbox_id"])):
        content = data["category"][id]
        coords = data["bbox"][id]
        coords = [int(i) for i in coords]
        quad_coords = ((coords[0], coords[1]), (coords[2], coords[3]))
...
def hit_score(attention_maxtrix, visual_head_score, inp, step_token, bbox_token_idx, topk=1):
    if(len(step_token) <= 2): # all categories are more than 2 characters
        return False
    flag = False
    ...
    for step_i in range(max_decode_len):
        ...
        for idx, meta_ocr in enumerate(ocr_idxs):
            if step_token in meta_ocr[0][0]:
                ...
                ret = hit_score(outputs.attentions, visual_head_score, inp, step_token, ocr_idx)
                if ret:
                    break # find the needle
...
def eval_single(idx, da):
    ...
    parsed_data = parse_coco_data(data_json['content'])
    image_tensor = process_images([image], image_processor, config)
    # import pdb;pdb.set_trace()

    question = "Describe objects in the picture."
...
def main():
    data_path = "/path/to/datasets/coco/coco_val.json"
    ...
    with open(f"./visual_head/head_score/{model_version}_coco.json", 'w') as f:
